        ...
        <View>
            {
                ...
                <TouchableOpacity style={[styles.wrapper,boxStyles]} onPress={() => { if(!dropdown){ slidedown() }else{ slideup() } }}>
                        <Text style={[{fontFamily},inputStyles]}>{ placeholder  }</Text>
                        {
                            (!arrowicon)
                            ?
                                <Image 
                                        ...
                                        if(disabled){
                                            ...
                                            return(
                                                <TouchableOpacity style={[styles.option,dropdownItemStyles]} key={index} onPress={ () => {
                                                    ...
                                                    const ada=selectedval.filter((dt)=>dt.key==key).length>0

                                                    if(ada){
                                                        const nui=selectedval.filter((dt)=>dt.key!=key);
                                                        setSelected(nui)
                                                        setSelectedVal(nui)
                                                    }else{
                                                        setSelected([...selectedval,item])
                                                        setSelectedVal([...selectedval,item])
                                                    }
                                                    ...
                                                    <View style={[{width:15,height:15,borderWidth:1,marginRight:10,borderColor:'gray',borderRadius:3,justifyContent:'center',alignItems:'center'},checkBoxStyles]}>
                                                        ...
                                                        {
                                                            (selectedval.filter((dt)=>dt.key==key).length>0)
                                                            ?
            ...
            {selectedval?.length>0?renderBadge?renderBadge(selectedval):null:null}
        </View>
    )
}
